...
-                  />
-                  <button
-                    onClick={() => { setLookupSearched(lookupQuery); /* actual handlers in AdminCMS will perform lookups */ }}
...
+              setLookupQuery={(v) => setLookupQuery(v)}
+              lookupSearched={lookupSearched}
+              setLookupSearched={(v) => setLookupSearched(v)}
+              onSearch={(q) => { /* AdminCMS handles actual search side effects */ }}
+            />
+          )}
@@
 (end of AdminCMS.tsx lookup-block replacement)
